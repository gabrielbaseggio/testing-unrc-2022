...
public class TaTeTi {
	...
	public boolean termino() {
		...
		return hayGanador || tablero.lleno();
	}
	...
	public Ficha ganador() {
		...
		for( int row = 0; row < 3; row++ ) {
			if( tablero.ficha(row, 0) != null && tablero.ficha(row, 0).equals( tablero.ficha(row, 1) ) && tablero.ficha(row, 1).equals( tablero.ficha(row, 2) ) ) return tablero.ficha(row, 0);
		}
		
		for( int col = 0; col < 3; col++ ) {
			if( tablero.ficha(0, col) != null && tablero.ficha(0, col).equals( tablero.ficha(1, col) ) && tablero.ficha(1, col).equals( tablero.ficha(2, col) ) ) return tablero.ficha(0, col);
		}
